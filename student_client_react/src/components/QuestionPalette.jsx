...
export default function QuestionPalette({
  ...
  return (
    <div className="bg-white border border-[#E2E8F0] rounded-2xl flex flex-col h-full shadow-md overflow-hidden">
      {/* 1. Student Header Profile Summary */}
      <div className="p-4 bg-[#F4F6F9] border-b border-[#E2E8F0] flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h4 className="font-bold text-xs sm:text-sm text-[#1E242B] truncate">{displayName}</h4>
          <p className="text-[11px] text-[#64748B] font-semibold truncate">
            Reg: <span className="font-mono">{student?.reg_number || 'N/A'}</span> • Class: {student?.class || 'N/A'}
          </p>
        </div>
      </div>
      ...
      <div className="p-4 border-b border-[#E2E8F0] space-y-2">
        <div className="flex items-center justify-between text-xs font-bold text-[#1E242B]">
        ...
        {/* Legend */}
        <div className="grid grid-cols-3 gap-1 pt-2 text-[10px] font-bold text-[#1E242B]">
          <div className="flex items-center gap-1.5 bg-orange-50 p-1 rounded border border-orange-200">
            <span className="w-2.5 h-2.5 rounded bg-[#F96302]"></span>
            <span>Answered</span>
          </div>
          <div className="flex items-center gap-1.5 bg-slate-100 p-1 rounded border border-slate-200 text-[#64748B]">
            <span className="w-2.5 h-2.5 rounded bg-[#E2E8F0] border border-slate-300"></span>
            <span>Pending</span>
          </div>
      ...
      <div className="flex-1 overflow-y-auto max-h-[380px] p-4">
        <div className="grid grid-cols-5 gap-2">
          {Array.from({ length: totalQuestions }, (_, i) => {
            ...
            let tileStyle = 'bg-[#F1F5F9] border-[#CBD5E1] text-[#1E242B] hover:bg-slate-200 font-semibold';
            ...
            return (
              <button
                ...
                className={`relative aspect-square rounded-xl text-xs flex items-center justify-center transition-all ${tileStyle}`}
      ...
      <div className="p-4 border-t border-[#E2E8F0]">
        <button
          ...
          className="w-full py-3 border-2 border-[#F96302] text-[#F96302] hover:bg-orange-50 font-extrabold text-xs rounded-xl transition-all flex items-center justify-center gap-2 uppercase tracking-wider"
        >
