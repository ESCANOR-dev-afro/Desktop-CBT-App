  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-darkBorder p-5 rounded-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-xl">
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-slate-950 hover:bg-slate-800 border border-darkBorder text-xs text-slate-300 font-bold transition-all flex items-center space-x-2 cursor-pointer"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3.5">
        <div className="bg-slate-900 border border-darkBorder p-4 rounded-xl flex items-center justify-between shadow-sm">
          <div>
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Enrolled</p>
            <p className="text-xl font-extrabold text-slate-100 mt-0.5">{metrics.enrolledCandidates}</p>
          </div>
        ...
        <div className="bg-slate-900 border border-darkBorder p-4 rounded-xl flex items-center justify-between shadow-sm">
          <div>
            ...
            <p className="text-xl font-extrabold text-emerald-400 mt-0.5">{metrics.activeWorkstations}</p>
          </div>
        ...
        <div className="bg-slate-900 border border-darkBorder p-4 rounded-xl flex items-center justify-between shadow-sm">
          <div>
            ...
            <p className="text-xl font-extrabold text-rose-400 mt-0.5">{metrics.lockoutAlerts}</p>
          </div>
        ...
        <div className="bg-slate-900 border border-darkBorder p-4 rounded-xl flex items-center justify-between shadow-sm">
          <div>
            ...
            <p className="text-xl font-extrabold text-slate-400 mt-0.5">{metrics.idleNodes}</p>
          </div>
      ...
      {/* Workstations Grid */}
      {sessions.length === 0 ? (
        <div className="bg-slate-900/60 border border-dashed border-darkBorder rounded-2xl p-12 text-center space-y-3">
          ...
          <h4 className="text-sm font-bold text-slate-200">No Active Exam Sessions</h4>
          <p className="text-xs text-slate-400 max-w-md mx-auto leading-relaxed">
            No active exam sessions in progress for <strong className="text-slate-200">{currentClass}</strong>. Workstation cards will appear live when students launch their paper.
          </p>
        </div>
      ) : (
              ...
              <div
                ...
                className={`bg-slate-900 border rounded-2xl p-4 flex flex-col justify-between space-y-3 transition-all relative overflow-hidden ${
                  isLockout
                    ...
                    : 'border-darkBorder opacity-80'
                }`}
              >
                {/* Header: Node & Status Badge */}
                <div className="flex items-center justify-between pb-2.5 border-b border-darkBorder/60">
                  ...
                  {isActive && (
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 flex items-center space-x-1">
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-[10px] text-slate-400 font-semibold">
                      ...
                      <span className="text-slate-200 font-mono">{formatTimeRemaining(ws.expiresAt)}</span>
                    </div>
                  ...
                  {/* Action Controls */}
                  {!isSubmitted && (
                    <div className="pt-2 border-t border-darkBorder/60 flex items-center space-x-1.5">
                      {isLockout ? (
                        <button
                        ...
                        <>
                          <button
                            ...
                            className="flex-1 py-1.5 px-2 bg-slate-950 hover:bg-slate-800 border border-darkBorder text-slate-300 rounded-lg text-[10px] font-semibold transition-colors flex items-center justify-center space-x-1 cursor-pointer"
                          >
                          ...
                          <button
                            ...
                            className="flex-1 py-1.5 px-2 bg-slate-950 hover:bg-slate-800 border border-darkBorder text-rose-400 rounded-lg text-[10px] font-semibold transition-colors flex items-center justify-center space-x-1 cursor-pointer"
                          >
